 ...
 */
- (void) ntSetFloatBtnVisible:(BOOL) v;
 ...
 *  @brief 用户支付订单的函数。
 *
 *  支付完成或取消后，视Sdk具体情况，一般会发出 NT_NOTIFICATION_CLOSE_PAYVIEW 通知
 *
 ...
 */
- (void) setProp:(NSString*) prop as:(id) val;
 ...
 *  @return 与传入名称对应的属性值，若传入nil，则删除相应的属性值
 */
- (void) setPropStr:(NSString*) prop as:(NSString*) val;
 ...
 */
- (NSString*) getPayChannelByPid: (NSString*)pid;

/**
 *  cc录像功能
 */
- (void)ntCCStartService; //打开窗口
- (void)ntCCStopService; //关闭窗口
- (UIImage *)ntCCTakeScreenShot;//截屏
- (NSInteger)getCCWindowState;//获取窗口显示的状态
- (BOOL)ntCCisSupportRecord;//设备是否支持录像
// G18 CC专用
      ...
      ccUploadCompleted:(void (^)(NSDictionary*))ccUploadCompleted
         ccUploadFailed:(void (^)(NSDictionary*))ccUploadFailed
          ccStartRecord:(void (^)(NSDictionary*))ccStartRecord
           ccStopRecord:(void (^)(NSDictionary*))ccStopRecord;
- (void)ntCCUploadVideoWithFileName:(NSString*)file_Name title:(NSString*)title desc:(NSString*)desc;
- (void)ntCCCancelUploadVideoWithFileName:(NSString*)file_Name;
- (void)ntCCUpdateVideoSharedStatus:(BOOL)hasShare byFileName:(NSString*)file_Name;
- (void)ntCCHideVideoListView;
- (BOOL)ntCCIsRecording;
- (void)ntCCStopRecord;
 ...
 * @brief 显示网页
 */
- (void)ntShowWeb:(NSString *)url;

/**
 * @brief 显示奖励页面
 */
- (void)ntShowRewardView:(NSArray *)rewardData;
/**
 * @brief appstore账号信息(家庭分享计划的账号)
 */
- (NSString *)ntHomeShareingAppleId;
 ...
 */
- (NSArray *)ntProductsList;

/**
 *  @brief  设置是否检查非常规支付
 *  @param dectectOrNot YES or NO
 */
- (void)ntSetIAPDetectIrregularActiviy:(BOOL)detectOrNot;
...
- (void)ntSelectChannelOption:(int)option;

/**
 * @brief 发送邀请（GameCenter功能）
 * @param identifiers 玩家ID列表
 */
- (void) ntInviteFriendList:(NSArray*) identifiers;
/**
 * @brief 获取默认排行榜信息（GameCenter功能）
 */
- (void) ntQueryDefaultRank;
/**
 * @brief 设置默认排行榜信息（GameCenter功能）
 * @param identifier 排行榜标识
 */
- (void) ntSetDefaultRank:(NSString *)identifier;
/**
 * @brief 展示标准排行榜（GameCenter功能）
 * @param identifier 排行榜标识
 */
- (void) ntDisplayLeaderboard:(NSString *)identifier;
/**
 ...
 */
- (void) ntQueryScores:(NSDictionary *)filter;
 ...
 */
- (void) ntUpdateAchievement:(NSString *)identifier forPlayer: (NSString *) playerID percentComplete: (float) percent;
/**
 * @brief 报告成就，默认当前登录的玩家（GameCenter功能）
 * @param identifier 排行榜标识
 * @param percent 进度，设置1-100
 */
- (void) ntUpdateAchievement:(NSString *)identifier percentComplete: (float) percent;

/**
 * @brief 展示成就（GameCenter功能）
 * @param identifier 排行榜标识
 */
- (void) ntDisplayAchievement:(NSString *)identifier;

/**
 * @brief 获取成就元数据(用于自定义成就界面)（GameCenter功能）
 * @param identifier 排行榜标识
 */
- (void) ntQueryAchievement:(NSString *)identifier;
 ...
 */
- (void) ntPrepareChallenge:(NSString *)identifier onScore:(BOOL)isScore withValue:(NSInteger)score;
 ...
 * @param message 消息
 */
- (void) ntIssueChallengeToPlayers:(NSArray*) identifiers message:(NSString *)message;

/**
 * @brief 报告挑战进度（GameCenter功能）
 *         挑战排行榜 value表示分数，不能超过预先设置的最高分；挑战成就 value表示百分比1-100
 */
- (void) ntUpdateChallenge:(NSInteger)value;

/**
 * @brief 获取挑战列表信息（GameCenter功能）
 */
- (void) ntQueryChallenge;
 ...
 */
- (void)ntSetSDKEnvironment:(int)code;
 ...
 * @brief 计费充值日志上传接口
 */
- (void) ntSaveLogToJFOnPay:(NtOrderInfo*)order;
/**
...
/**
 * @brief 内嵌UIWebview视图打开网页，用于问卷调查等
 * @param URL: 网页地址
 */
 ...
 *  设置计费sauth要用到的字段及内容
 */
- (void)setJFSauthWithKey:(NSString *)key andValue:(NSString *)value urlEncode:(BOOL)encode;
